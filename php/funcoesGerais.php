<?php
  ...
  function recepcao() {
    ...
    /* Listamos os tecnicos referentes ao supervisor em questão */
    require ('conectarBD.php');
    $stmt = $conexao_pdo->prepare("SELECT * FROM retornos WHERE (status != 'Finalizado' AND status != 'Agendado') AND setor ='fale conosco'");
      $stmt->execute();
      $result = array();
      while($r = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $result[] = $r;
      ...
      function faleConoscoAgendado() {
    /* Listamos os tecnicos referentes ao supervisor em questão */
    require ('conectarBD.php');
    $stmt = $conexao_pdo->prepare("SELECT * FROM retornos WHERE status = 'Agendado' AND setor ='fale conosco'");
                $stmt->execute();
                $result = array();
                while($r = $stmt->fetch(PDO::FETCH_ASSOC)) {
                  $result[] = $r;
